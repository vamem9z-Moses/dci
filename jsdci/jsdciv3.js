...
Context.prototype = {
  assignRole: function assignRole(rolePrototype, obj) {
    var methods = Object.getOwnPropertyNames(rolePrototype);
    methods.map(function(method) {
      obj[method] = rolePrototype[method]
    });
  },
  removeRole: function removeRole(rolePrototype, obj) {
    var methods = Object.getOwnPropertyNames(rolePrototype);
    methods.map(function(method) {
     delete obj[method]
    });
...
var AccountDepositContext = function AccountDepositContext(accountDomain, message, entryTime, amount) {
  ...
  self.amount = amount;
  self.assignRole(AccountRole.prototype, self.account);
  return self;
};
...
var TransferMoneyContext = function TransferMoneyContext(sourceAccount, destAccount, amount) {
  ...
  self.amount = amount;
  self.assignRole(TransferMoneySource.prototype, self.source);
  return self;
};
